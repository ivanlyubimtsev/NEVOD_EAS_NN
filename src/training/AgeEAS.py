...
with open('C:/Users/lia/PycharmProjects/НИР/spe27p_100k_2022_correct.dat', 'rb') as binary_file:
    angle_phi = []
    angle_tetta = []
    coordinates_x = []
    coordinates_y = []
    ...
    #time, age = without_zero_events(time, age)
    ...
    age = torch.tensor(age).unsqueeze_(1).float()
    ...
    for m in range(10000):
        ...
        binary_file.tell()
        data_byte = binary_file.read(4 * p5)
        power_eas_V = struct.unpack('f' * p5, data_byte)[0]
        power_V.append(math.log10(power_eas_V))

        q6, p6 = 0, 1
        data_byte = binary_file.read(4 * q6)
        binary_file.tell()
        data_byte = binary_file.read(4 * p6)
        age_eas_V = struct.unpack('f' * p6, data_byte)[0]
        age_V.append(age_eas_V)
    ...
    #time_V, age_V = without_zero_events(time_V, age_V)
    ...
    age_V = torch.tensor(age_V).unsqueeze_(1).float()
    ...
    for n in range(10000):
        ...
        binary_file.tell()
        data_byte = binary_file.read(4 * p5)
        power_eas_T = struct.unpack('f' * p5, data_byte)[0]
        power_T.append(math.log10(power_eas_T))

        q6, p6 = 0, 1
        data_byte = binary_file.read(4 * q6)
        binary_file.tell()
        data_byte = binary_file.read(4 * p6)
        age_eas_T = struct.unpack('f' * p6, data_byte)[0]
        age_T.append(age_eas_T)
    ...
    #time_T, age_T = without_zero_events(time_T, age_T)
    ...
    age_T = torch.tensor(age_T).unsqueeze_(1).float()
...
optimizer = torch.optim.Adam(EAS_net.parameters(), lr=1.0e-3)
...
for epoch in range(2):
    ...
    for start_index in range(0, len(energy_plus_time), batch_size):
        ...
        age_batch = age[batch_indexes].to(device)

        preds = EAS_net.forward(energy_plus_time_batch)

        loss_value = loss(preds, age_batch)
    ...
    train_loss_history.append(loss(preds, age).item())
    val_loss_history.append(loss(val_preds, age_V).item())

    train_accuracy = (age - preds).std()
    val_accuracy = (age_V - val_preds).std()
...
preds = EAS_net.forward(energy_plus_time_T)
preds = preds.T
preds = preds[0].tolist() #Why 0
age_T = age_T.T
age_T = age_T[0].tolist()
subtract = np.subtract(age_T, preds).tolist()

fig, ax = plt.subplots()
#pylab.subplot (2, 1, 2)
plt.hist(subtract, bins=150)
plt.xlabel('Разность моделированных и восстановленных значений возраста ШАЛ', fontsize=20)
...
fig, ax = plt.subplots()
#pylab.subplot (2, 1, 2)
plt.hist(age_T, bins=150)
plt.xlabel('Моделированные значения возраста ШАЛ', fontsize=20)
plt.ylabel('Количество событий', fontsize=20)
ax.tick_params(axis='x', labelsize=16)
ax.tick_params(axis='y', labelsize=16)
#pylab.subplot (2, 1, 2)
